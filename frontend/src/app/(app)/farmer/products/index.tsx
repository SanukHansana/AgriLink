  ...
  return (
    <SafeAreaView edges={['top']} style={styles.safeArea}>
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl onRefresh={refresh} refreshing={isLoading} />}>
        ...
        <View style={styles.listHeader}><View style={styles.filters}>{(['all', 'active', 'pending', 'sold'] as StatusFilter[]).map((item) => <Pressable key={item} onPress={() => setFilter(item)} style={[styles.filter, filter === item && styles.activeFilter]}><Text style={[styles.filterText, filter === item && styles.activeFilterText]}>{item === 'all' ? 'All' : item.charAt(0).toUpperCase() + item.slice(1)}</Text></Pressable>)}</View><Pressable onPress={() => router.push('/(app)/farmer/bids' as Href)}><Text style={styles.bidsLink}>Received Bids</Text></Pressable></View>
...
const styles = StyleSheet.create({
  ...
  listHeader: { alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between' }, filters: { flexDirection: 'row', gap: 7, marginVertical: 14 }, filter: { backgroundColor: BrandColors.primarySoft, borderRadius: 17, paddingHorizontal: 11, paddingVertical: 8 }, activeFilter: { backgroundColor: BrandColors.primary }, filterText: { color: BrandColors.primary, fontSize: 9, fontWeight: '800' }, activeFilterText: { color: BrandColors.white }, bidsLink: { color: BrandColors.primary, fontSize: 9, fontWeight: '800' }, grid: { flexDirection: 'row', flexWrap: 'wrap', gap: 10 },
});
